...
const config = {
    ...
    server: process.env.DB_HOSTNAME, // You can use 'localhost\\instance' to connect to named instance
...
passport.use(new LocalStrategy({
    ...
    function(req, email, password, done) { // callback with email and password from our form
        new sql.Request().query(`SELECT * FROM users WHERE email = '${email}'`, function(err, rows){
            if (err) {
                return done(err);
            }
            console.log(rows);
            ...
            const hash = rows.recordset[0].password.toString();
...
passport.deserializeUser(function(id, done) {
    new sql.Request().query(`SELECT id, email, username, imageurl, level FROM users WHERE id = ${id}`, function(err,rows){
