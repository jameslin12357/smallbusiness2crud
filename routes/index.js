...
router.get('/customers', function(req, res, next) {
    if (req.isAuthenticated()) {
        new sql.Request().query('SELECT * FROM customers ORDER BY date_created DESC; SELECT count(*) as count FROM customers',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/customers', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            new sql.Request().query(`INSERT INTO customers (first_name, last_name, age, dob, email, phone_number, gender_id, address_id) VALUES ('${first_name}','${last_name}','${age}','${dob}','${email}','${phone_number}','${gender_id}','${address_id}')`,
                function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.get('/customers/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        new sql.Request().query(`SELECT id, first_name, last_name, age, dob, email, phone_number, gender_id, address_id FROM customers WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/customers/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const gender_id = req.body.gender_id;
        const address_id = req.body.address_id;
        new sql.Request().query(`UPDATE customers SET first_name = '${first_name}', last_name = '${last_name}', age = '${age}', dob = '${dob}',
...
router.delete('/customers/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        new sql.Request().query(`DELETE FROM customers WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/customersstaffs', function(req, res, next) {
    if (req.isAuthenticated()) {
        new sql.Request().query('SELECT * FROM customersstaffs ORDER BY date_created DESC; SELECT count(*) as count FROM customersstaffs',
...
router.post('/customersstaffs', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            new sql.Request().query(`INSERT INTO customersstaffs (customer_id, staff_id) VALUES ('${customer_id}','${staff_id}')`,
                function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.get('/customersstaffs/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        new sql.Request().query(`SELECT id, customer_id, staff_id FROM customersstaffs WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/customersstaffs/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const customer_id = req.body.customer_id;
        const staff_id = req.body.staff_id;
        new sql.Request().query(`UPDATE customersstaffs SET customer_id = '${customer_id}', staff_id = '${staff_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/customersstaffs/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        new sql.Request().query(`DELETE FROM customersstaffs WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/customersstaffsinventories', function(req, res, next) {
    if (req.isAuthenticated()) {
        new sql.Request().query('SELECT * FROM customersstaffsinventories ORDER BY date_created DESC; SELECT count(*) as count FROM customersstaffsinventories',
            function (error, results, fields) {
                if (error) {
                    throw error;
                }
...
router.post('/customersstaffsinventories', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            new sql.Request().query(`INSERT INTO customersstaffsinventories (customersstaffs_id, inventories_id) VALUES ('${customersstaffs_id}','${inventories_id}')`,
                function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.get('/customersstaffsinventories/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        new sql.Request().query(`SELECT id, customersstaffs_id, inventories_id FROM customersstaffsinventories WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/customersstaffsinventories/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const customersstaffs_id = req.body.customersstaffs_id;
        const inventories_id = req.body.inventories_id;
        new sql.Request().query(`UPDATE customersstaffsinventories SET customersstaffs_id = '${customersstaffs_id}', inventories_id = '${inventories_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/customersstaffsinventories/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        new sql.Request().query(`DELETE FROM customersstaffsinventories WHERE id = '${req.params.id}'`,function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/inventories', function(req, res, next) {
    if (req.isAuthenticated()) {
        new sql.Request().query('SELECT * FROM inventories ORDER BY date_created DESC; SELECT count(*) as count FROM inventories',
            function (error, results, fields) {
                if (error) {
                    throw error;
                }
...
router.post('/inventories', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            new sql.Request().query(`INSERT INTO inventories (name, description, stock, manufacturer_id) VALUES ('${name}','${description}','${stock}','${manufacturer_id}')`,
                function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.get('/inventories/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        new sql.Request().query(`SELECT id, name, description, stock, manufacturer_id FROM inventories WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/inventories/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const stock = req.body.stock;
        const manufacturer_id = req.body.manufacturer_id;
        new sql.Request().query(`UPDATE inventories SET name = '${name}', description = '${description}', stock = '${stock}', manufacturer_id = '${manufacturer_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/inventories/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        new sql.Request().query(`DELETE FROM inventories WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/manufacturers', function(req, res, next) {
    if (req.isAuthenticated()) {
        new sql.Request().query('SELECT * FROM manufacturers ORDER BY date_created DESC; SELECT count(*) as count FROM manufacturers',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/manufacturers', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            new sql.Request().query(`INSERT INTO manufacturers (name, contact_number, contact_person_first_name, contact_person_last_name) VALUES ('${name}','${contact_number}','${contact_person_first_name}','${contact_person_last_name}')`,
                function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.get('/manufacturers/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        new sql.Request().query(`SELECT id, name, contact_number, contact_person_first_name, contact_person_last_name FROM manufacturers WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/manufacturers/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const contact_person_first_name = req.body.contact_person_first_name;
        const contact_person_last_name = req.body.contact_person_last_name;
        new sql.Request().query(`UPDATE manufacturers SET name = '${name}', contact_number = '${contact_number}', contact_person_first_name = '${contact_person_first_name}', contact_person_last_name = '${contact_person_last_name}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/manufacturers/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        new sql.Request().query(`DELETE FROM manufacturers WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/staffs', function(req, res, next) {
    if (req.isAuthenticated()) {
        new sql.Request().query('SELECT * FROM staffs ORDER BY date_created DESC; SELECT count(*) as count FROM staffs',
            function (error, results, fields) {
                if (error) {
                    throw error;
                }
...
router.post('/staffs', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            const gender_id = req.body.gender_id;
            const address_id = req.body.address_id;
            new sql.Request().query(`INSERT INTO staffs (first_name, last_name, age, dob, email, phone_number, gender_id, address_id) VALUES ('${first_name}','${last_name}','${age}','${dob}','${email}','${phone_number}','${gender_id}','${address_id}')`,
                 function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.get('/staffs/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        new sql.Request().query(`SELECT id, first_name, last_name, age, dob, email, phone_number, gender_id, address_id FROM staffs WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/staffs/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const gender_id = req.body.gender_id;
        const address_id = req.body.address_id;
        new sql.Request().query(`UPDATE staffs SET first_name = '${first_name}', last_name = '${last_name}', age = '${age}', dob = '${dob}',
            email = '${email}', phone_number = '${phone_number}', gender_id = '${gender_id}', address_id = '${address_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
...
router.delete('/staffs/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        new sql.Request().query(`DELETE FROM staffs WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
